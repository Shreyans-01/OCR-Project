...
def openImage():
    ...
    imgName = filedialog.askopenfilename(initialdir='C:/Users/onesh/OneDrive/Desktop/OpenCV/RDBMS-IA2',
                                         title='Select Image', filetypes=(('JPG', '*.jpg'), ('All files', '*.*')))
...
def warpImage():
    ...
    # assuming clicks have been made in order top left -> top right -> bottom left -> bottom right
    topLef = clicks[0]
    topRig = clicks[1]
    botLef = clicks[2]
    botRig = clicks[3]
...
def getText():
    ...
    thresh = cv2.adaptiveThreshold(
        croppedGrey, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 201, 35)
...
text = tk.Text(centerFrame, bg='gray')
text.place(relx=0.5, rely=0.4, relwidth=0.75, relheight=0.5, anchor='n')

openImage = tk.Button(canvas, text='Open Image', padx=10,
                      pady=10, command=openImage)
openImage.place(relx=0.1, rely=0.05, anchor='n')

# autoCrop = tk.Button(canvas, text='Auto Crop', padx=10,
#                      pady=10, command=autoCrop)
# autoCrop.place(relx=0.1, rely=0.3, anchor='n')

manualCrop = tk.Button(canvas, text='Manual Crop',
                       padx=10, pady=10, command=manualCrop)
manualCrop.place(relx=0.1, rely=0.5, anchor='n')

saveImage = tk.Button(canvas, text='Save Image', padx=10,
                      pady=10, command=saveImage)
saveImage.place(relx=0.1, rely=0.9, anchor='n')

getText = tk.Button(canvas, text='Detect Text',
                    padx=10, pady=10, command=getText)
getText.place(relx=0.9, rely=0.05, anchor='n')

showText = tk.Button(canvas, text='Show Text', padx=10,
                     pady=10, command=showText)
showText.place(relx=0.9, rely=0.3, anchor='n')

closeWindows = tk.Button(canvas, text='Close All Windows',
                         padx=10, pady=10, command=closeAllWindows)
